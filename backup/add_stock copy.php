                                        ...
                                        while ($result = mysqli_fetch_array($query)) { ?>
                                            <div class="card mx-2 my-2 text-center" style="width: 200px;">
                                                ...
                                                <div class="card-footer">
                                                    <a href="#" class="btn btn-success" onclick="addToStockCutBox('<?= $result["Product_Name"]; ?>', <?= $result["Product_Qty"]; ?>)">เพิ่มสต็อกสินค้า</a>
                                                </div>
                                            </div>
                                        <?php } ?>
                                    </div>
                        ...
                        <div class="col-lg-5 pt-5">
                            <div class="row gy-3 mb-4">
                                <form method="POST">
                                    ...
                                    <button type="submit" class="btn btn-success w-100 shadow-0 mb-2" name="update_stock">เพิ่มสต็อกสินค้า</button>
                                    <a href="#" class="btn btn-light w-100 border mt-2">ย้อนกลับ</a>
                                </form>
                            </div>
                        </div>
                        <!-- cut stock box -->
...
require_once 'connection.php';
    ...
    if (isset($_POST["cut_stock"])) {
        // Retrieve the selected products for stock cutting from the POST data
        $selectedProducts = $_POST["selected_products"];

        // Loop through the selected products and update the stock in the database
        foreach ($selectedProducts as $productName => $quantityToCut) {
            ...
            if ($result) {
                ...
                Swal.fire({
                  icon: 'error',
                  title: 'Oops...',
                  text: 'ไม่สามารถตัดสต๊อกสินค้าได้ โปรดลองใหม่อีกครั้งนึง',
                });
                </script>
                <?php
              }
