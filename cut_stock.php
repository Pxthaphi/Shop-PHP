...
<head>
    ...
    $menu = "cut_stock";
    ?>
    <title>ตัดสต๊อกสินค้า | ร้านมาลีวัลย์สังฆภัณฑ์</title>
    <!-- MDB -->
...
<body>
                                            ...
                                            ?>
                                                ...
                                                <div class="card mx-2 my-2 text-center" style="width: 200px;">
                                                    ...
                                                    <div class="card-footer">
                                                        <button class="btn btn-danger" onclick="addToStockCutBox('<?= $result["Product_Name"]; ?>', <?= $result["Product_Qty"]; ?>, <?= $result["Product_ID"]; ?>)">ตัดสต็อกสินค้า <i class="bi bi-bag-dash-fill"></i></button>
                                                    </div>
                                                </div>
                                            <?php } ?>
                                        </div>
                            ...
                            <div class="col-lg-5 pt-5">
                                <div class="row gy-3 mb-4">
                                    <form method="POST" onsubmit="return validateForm();">
                                        ...
                                        <button type="submit" class="btn btn-success w-100 shadow-0 mb-2" id="cutStockButton" name="cut_stock">ตัดสต็อกสินค้า <i class="bi bi-bag-dash-fill"></i></button>
    ...
    <script>
        function addToStockCutBox(productName, currentQuantity, productID) {
            ...
            if (!existingProduct) {
                ...
                productQuantityInput.addEventListener("input", function() {
                    // Clear the custom validation message on input chang
        ...
        function validateForm() {
            ...
            if (selectedProductsCount === 0) {
                Swal.fire({
                    ...
                    text: 'กรุณาเลือกสินค้าก่อนตัดสต๊อก!!',
...
if (isset($_POST["cut_stock"])) {
    ...
    foreach ($selectedProducts as $productName => $productData) {
        ...
        $quantityToCut = intval($productData['quantity']);
        $productID = intval($productData['product_id']);

        // Update the stock in the database
        $sql = "UPDATE product SET Product_Qty = Product_Qty - $quantityToCut WHERE Product_ID = $productID";
    ...
    if ($allUpdatesSuccessful) {
        ...
        $actionType = 'ตัดสต๊อก'; // Example value, replace with your actual data

        $sqlHistory = "INSERT INTO sales (Date, Action_Type) 
                        VALUES ('$date', '$actionType')";
        $queryHistory = mysqli_query($conn, $sqlHistory);

        if ($queryHistory) {
            // Insert data into 'sales_product' table using the generated History_ID
            ...
            foreach ($selectedProducts as $productName => $productData) {
                ...
                $sqlHistoryProduct = "INSERT INTO sales_product (Sales_ID, Pro_ID, Username, Product_Qty) 
                                        VALUES ('$salesID', '$productID', '$username', '$quantityToCut')";
