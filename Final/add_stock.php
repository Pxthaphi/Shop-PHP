...
<body>
                                            ...
                                            while ($result = mysqli_fetch_array($query)) { ?>
                                                <div class="card mx-2 my-2 text-center" style="width: 200px;">
                                                    ...
                                                    <div class="card-footer">
                                                        <a href="#" class="btn btn-success" onclick="addToStockAddBox('<?= $result["Product_Name"]; ?>', <?= $result["Product_Qty"]; ?>)">เพิ่มสต็อกสินค้า</a>
                                                    </div>
                                                </div>
                                            <?php } ?>
                                        </div>
                                ...
                                <div class="row gy-3 mb-4">
                                    <form method="POST" onsubmit="return validateForm();">
                                        <div id="stock-add-box">
                                            <!-- Selected products for stock addition will be displayed here -->
                                        </div>
    ...
    <script>
        function addToStockAddBox(productName, currentQuantity) {
            ...
            if (!existingProduct) {
                ...
                deleteButton.onclick = function() {
                    ...
                    // After removing a product, update the hidden input field with selected products and quantities
...
if (isset($_POST["add_stock"])) {
    ...
    foreach ($selectedProducts as $productName => $quantityToAdd) {
        // Validate and sanitize the input (you should implement proper input validation)
        $productName = mysqli_real_escape_string($conn, $productName);
        $quantityToAdd = intval($quantityToAdd);

        // Update the stock in the database
        $sql = "UPDATE product SET Product_Qty = Product_Qty + $quantityToAdd WHERE Product_Name = '$productName'";
